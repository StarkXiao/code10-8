...
Playwright 端到端覆盖 8 条真实浏览器链路：注册建档、上传照片 + 画布标记、列表缩略图、打点穿着、**破损→修补→复检闭环**、全部页面冒烟（含图表与字典页）、布料建档 + 只读/协作分享链接（师傅回填用料费用 → 主人确认并入档案）、断网打点离线入队与自动同步。

---

## 限时协作：从只读到「师傅回填、主人确认」

分享链接有两种权限（均带有效期，随时可撤销）：

- **只读**：访客能看照片、破损与修补史，看不到成本与收纳位置等隐私字段；
- **限时协作**：师傅可在链接有效期内，对着某条未闭环的破损**回填针法、用料明细与费用**，提交后生成一张「回填单」。

回填不会直接进档案，流程是：

```
主人发协作链接（限时） → 师傅回填用料/费用
   ↓
生成 ShareIntake 回填单（pending，永不删除）+ 站内/SSE 提醒主人
   ↓
主人审核：补选针法、核对费用、把用料逐行映射到库存布料
   ├── 确认 → 同一事务内生成正式 Repair（映射用料扣库存、未映射用料进备注）
   │          回填单置 confirmed 并挂上 repairId，提醒闭环
   └── 驳回 → 必填原因，师傅在链接有效期内修正后可重新提交
```

完整操作痕迹保留在审计日志：访客提交记为 `share:<链接id> / 访客·分享链接`，主人确认/驳回分别记 `confirm` / `reject`，并入的修补日志带来源标记 `source: share_intake`；库存流水注明「协作回填并入」。双击或并发确认不会并出两条修补（事务内条件抢占单据）。库存不足时整笔回滚，单据保持待确认。
